...
# read CSV file
Deaths.dt <- fread(here::here("data", "outputs", "Deaths", "All_Deaths_Storm_Effects.csv"))
Deaths.df <- Deaths.dt |>
  as.data.frame()

# read gpkg
Deaths.sf <- st_read(here::here("data", "outputs", "Deaths", "All_Deaths_Storm_Effects.gpkg"),
...
# Any points not in a polygon? (e.g., offshore or boundary issues)
sum(is.na(deaths_with_zip$GEOID20))
# returns 9753 (~.16%) when st_within or st_intersects. Could snap unmatched points to nearest polygon

# Remove deaths not inside a polygon
