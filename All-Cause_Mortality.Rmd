...
Read the CSV as a data table and/or the gpkg file
```{r}
library(sf)
library(data.table)

# read CSV file
Deaths.dt <- fread(here::here("data", "outputs", "Deaths", "Cardio_Deaths_Storm_Effects.csv"))
Deaths.df <- Deaths.dt |>
  as.data.frame()

# read gpkg
Deaths.sf <- st_read(here::here("data", "outputs", "Deaths", "Cardio_Deaths_Storm_Effects.gpkg"),
...
This tells us that in this sample the statewide daily death rate is somewhat higher on storm-effect days. But a lot is going on: time and spatial trends, seasonality, autocorrelation. We need a model that accounts for these things
...
# 1) National ZCTAs + Florida in same CRS
zctas_all <- zctas(cb = TRUE, year = 2020)
florida   <- states(cb = TRUE, year = 2020) %>%
  filter(STUSPS == "FL") %>%
  st_transform(st_crs(zctas_all))

# 2) Keep only ZCTAs whose *centroid* lies within Florida
zctas_cent <- st_centroid(zctas_all)

inside_fl  <- st_within(zctas_cent, florida, sparse = FALSE)[, 1]

zctas_fl <- zctas_all[inside_fl, ] %>%
  select(GEOID20, geometry)

# Sanity checks
nrow(zctas_fl)             # should be 1044
sum(st_is_empty(zctas_fl)) # should be 0
table(st_geometry_type(zctas_fl))

```

Spatial join deaths to zips
```{r}
# Transform deaths into ZCTA CRS
Deaths_aligned <- st_transform(Deaths.sf, st_crs(zctas_fl))

sf_use_s2(TRUE)

# Spatial join: add GEOID20 to each death
...
For all deaths the coefficients on the fixed effects are
...
Note: for cardio deaths the coefficients on the fixed effects are
Standard-errors: Clustered (zip & ym) 
                Estimate Std. Error  z value Pr(>|z|)    
effect::Threat  0.006818   0.052360 0.130214 0.896397    
effect::Impact  0.100527   0.059740 1.682755 0.092423 .  
effect::Cleanup 0.109005   0.049295 2.211266 0.027017 *
Log-Likelihood: -1,015,235.5   Adj. Pseudo R2: 0.123767
           BIC:  2,050,574.1     Squared Cor.: 0.046053
...
Note: When excluding weaker tropical storms 34-55 kt, the effect sizes are about twice as large 
