...
If we want avg_er_visits only when all dates have ER data, set non-full coverage to NA:
```{r}
er_avg_by_effect_strict <- er_match_long %>%
  group_by(GEOID20, Storm_Effect) %>%
  summarise(
    n_er_rows = sum(!is.na(Total_Visits)),
    n_dates   = first(n_dates),
    coverage  = ifelse(n_dates > 0, n_er_rows / n_dates, NA_real_),
    avg_er_visits = ifelse(coverage == 1, mean(Total_Visits, na.rm = TRUE), NA_real_),
    .groups = "drop"
  )
```
