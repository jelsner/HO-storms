...
# 1) Ensure geometry is valid and non-empty
Tracks.sf <- Tracks.sf |> 
...
Tracks.sf <- st_make_valid(Tracks.sf)

# 2) Build a clean distance vector in meters
# - coerce to numeric
# - replace remaining NA with a fallback (e.g., median RMW)
# - guard against non-positive values with a minimum
rmw_km <- suppressWarnings(as.numeric(Tracks.sf$USA_RMW)) * 1.852  # convert to km if you prefer
# Or stay in nautical miles and convert later

# Fallbacks
fallback_rmw_nm <- median(Tracks.sf$USA_RMW, na.rm = TRUE)          # in nautical miles
fallback_rmw_nm <- ifelse(is.finite(fallback_rmw_nm), fallback_rmw_nm, 20)  # use 20 nm if all NA

rmw_nm_clean <- Tracks.sf$USA_RMW
rmw_nm_clean[!is.finite(rmw_nm_clean)] <- fallback_rmw_nm
rmw_nm_clean <- pmax(rmw_nm_clean, 1)   # minimum radius 1 nm to avoid zeros/negatives

buf_m <- rmw_nm_clean * 1852            # meters

# 3) Perform the buffer
# CRS is 32616 (meters), so planar buffering is appropriate
sf_use_s2(FALSE)  # GEOS planarity for buffer
Swaths.sf <- st_buffer(Tracks.sf, dist = buf_m)
...
# Make the sf object the LHS, so geometry survives the join
TIC.sf <- Swaths_geom |>
  dplyr::left_join(exp_dates, by = "SID")

# TIC.sf is already sf (no need to call st_as_sf)
sf::st_crs(TIC.sf)  # should be EPSG:4326
...
NOTE: 7/0/0 fixed-effects (26,964 observations) removed because of only 0 outcomes or singletons.

Poisson estimation, Dep. Var.: deaths
Observations: 3,759,634
Fixed-effects: zip: 976,  ym: 126,  dow: 7
Standard-errors: Clustered (zip & ym) 
                 Estimate Std. Error   z value Pr(>|z|) 
effect::Threat   0.028643   0.019480  1.470418  0.14145 
effect::Impact  -0.012052   0.016384 -0.735592  0.46198 
effect::Cleanup  0.002312   0.016010  0.144408  0.88518 
---
Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
Log-Likelihood: -3,028,436.4   Adj. Pseudo R2: 0.170046
           BIC:  6,073,678.0     Squared Cor.: 0.242331
