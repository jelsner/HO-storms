...
Export (optional)
```{r}
readr::write_csv(panel, "data/outputs/Deaths/daily_panel_64kt_age80.csv")
...
```{r}
saveRDS(m_event, file = "models/m_event_64kt_age80.rds")
```
...
res_pre_64kt_groups <- bind_rows(
  ...
  summarize_pre_only(m_event_64kt_age80,  "Age ≥ 80", pre_days)
) |>
  mutate(group = factor(group, levels = c("All ages", "Age ≥ 80")))
