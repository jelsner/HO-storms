...
34 kt
1985 1987 1988 1990 1991 1992 1994 1995 1996 1997 1998 1999 
   5    1    2    1    1    1    3    4    1    1    3    2 
2000 2001 2002 2004 2005 2006 2007 2008 2009 2010 2011 2012 
   2    3    3    5    6    2    1    1    2    2    1    3 
...
rel_day_f
    None       -8       -7       -6       -5       -4       -3 
14286431    13587    13587    13588    13589    13589    13589 
      -2       -1        0        1        2        3        4 
   13589    13589    13589    13589    13589    13589    13589 
       5        6        7        8 
   13589    13588    13587    13587 
...
# ---- Averaging function (from your code, unchanged) ----
avg_lags <- function(m, days, prefix = "rel_day_f::", include_missing_as_zero = TRUE){
  beta <- coef(m)
  V    <- vcov(m)
...
# ---- Plot function: Main vs Placebo for PRE only (two-layer points + optional limits) ----
plot_pre_placebo_comparison <- function(main_model, placebo_model, main_label,
  ...
  # Per-day coefficients (assumes all terms present)
  ...
  p <- ggplot2::ggplot() +
    ...
    ggplot2::scale_color_gradient(low = "grey75", high = "grey10") +

    ggplot2::labs(
      x = NULL,
      y = "Pre-storm average RR (95% CI)",
      title = if (is.null(title)) paste0("Pre-storm daily RRs and average: ", main_label) else title,
      subtitle = "Daily dots (−8…−1) jittered; darker = closer to impact day (−1)"
    ) +
    ggplot2::theme_minimal()

  # Apply linear or log scale
  if (use_log_y) {
    p <- p + ggplot2::scale_y_log10()
  }

  # If y_limits provided, apply them consistently
  if (!is.null(y_limits)) {
    # coord_cartesian to avoid dropping data outside limits (and keep errorbar calc intact)
    p <- p + ggplot2::coord_cartesian(ylim = y_limits)
  }

  # Return plot + the underlying data (so we can compute global limits outside)
...
# ---- Build the three figures (first pass, without limits) ----
out_34 <- plot_pre_placebo_comparison(
  main_model    = m_event_34kt,
  placebo_model = m_event_34kt_placebo,
  main_label    = "≥34 kt",
  pre_days      = pre_days,
  use_log_y     = FALSE   # set TRUE if you prefer log scale
)

out_55 <- plot_pre_placebo_comparison(
  main_model    = m_event_55kt,
  placebo_model = m_event_55kt_placebo,
  main_label    = "≥55 kt",
  pre_days      = pre_days,
  use_log_y     = FALSE
)
...
# ---- Compute common y-limits across ALL panels (averages + daily points) ----
get_range <- function(out) {
  rng_days <- range(out$res_days$RR, na.rm = TRUE)
  rng_avg  <- range(c(out$res_avg$RR_L, out$res_avg$RR_U), na.rm = TRUE)
  range(c(rng_days, rng_avg), na.rm = TRUE)
}

rng_34 <- get_range(out_34)
rng_55 <- get_range(out_55)
rng_64 <- get_range(out_64)

global_ylim <- range(c(rng_34, rng_55, rng_64), na.rm = TRUE)

# Add a small padding for aesthetics on linear scale
pad <- 0.02
global_ylim <- c(
  max(0, global_ylim[1] - pad),   # RR can't be negative; keep >= 0
  global_ylim[2] + pad
)

# ---- Rebuild plots with the common y-limits applied ----
p_34 <- plot_pre_placebo_comparison(
  main_model    = m_event_34kt,
  placebo_model = m_event_34kt_placebo,
  main_label    = "≥34 kt",
  pre_days      = pre_days,
  use_log_y     = FALSE,
  y_limits      = global_ylim
)$plot

p_55 <- plot_pre_placebo_comparison(
  main_model    = m_event_55kt,
  placebo_model = m_event_55kt_placebo,
  main_label    = "≥55 kt",
  pre_days      = pre_days,
...
p_64 <- plot_pre_placebo_comparison(
  main_model    = m_event_64kt,
  placebo_model = m_event_64kt_placebo,
  main_label    = "≥64 kt",
  pre_days      = pre_days,
...
# Side-by-side with shared guides (colors are not shown in legend anyway)
library(patchwork)
p_34 + p_55 + p_64 + plot_layout(guides = "collect")
