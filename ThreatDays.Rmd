---
title: "Daily death counts prior to storm impact"
...
storm_intensity <- 34
begin_year <- 1981 # 1985 for 55+ winds, 1981 for 34+ winds
end_year <- 2022
...
1981 1982 1983 1984 1985 1987 1988 1990 1991 1992 1994 1995 
   1    1    1    2    5    1    2    1    1    1    3    4 
1996 1997 1998 1999 2000 2001 2002 2004 2005 2006 2007 2008 
   1    1    3    2    2    3    3    5    6    2    1    1 
2009 2010 2011 2012 2013 2016 2017 2018 2019 2020 2021 2022 
   2    2    1    3    1    3    3    3    1    3    4    3 
   
1985 1987 1988 1990 1992 1994 1995 1996 1997 1998 1999 2000 
   4    1    1    1    1    2    3    1    1    3    1    1 
2001 2004 2005 2006 2008 2012 2013 2016 2017 2018 2020 2021 
   2    4    5    1    1    1    1    1    1    1    3    2 
2022 
   2 
   
1985 1987 1992 1995 1997 1998 1999 2004 2005 2016 2017 
   2    1    1    2    1    2    1    4    4    1    1 
2018 2020 2022 
   1    1    2 
...
min_gap_days <- 30  # keep placebo ≥30 days away from real Date0

draw_placebo_far <- function(y, real_d, season_start="05-01", season_end="11-30",
                             lead=14, lag=10, margin=TRUE, min_gap=30) {
...
# Sanity check: differences should rarely be 0
table(Swaths_random.sf$Date0 - Swaths_random.sf$Original_Date0)
```
...
rel_day_f
    None       -8       -7       -6       -5       -4       -3 
14445048     4258     4258     4258     4258     4258     4258 
      -2       -1        0        1        2        3        4 
    4258     4258     4258     4258     4258     4258     4258 
       5        6        7        8 
    4258     4258     4258     4258 
...
Save
```{r}
saveRDS(deaths_with_zip, "data/outputs/Results/deaths_with_zip_64kt.rds")
deaths_with_zip<- readRDS("data/outputs/Results/deaths_with_zip_55kt.rds")
```
...
Export (optional)
```{r}
readr::write_csv(panel, "data/outputs/Deaths/daily_panel_64kt.csv")
...
```{r}
saveRDS(m_event_55kt2, file = "m_event_55kt2.rds")
...
Both main and placebo results
```{r}
# You may already have these loaded elsewhere
library(ggplot2)
library(dplyr)
library(broom)

plot_rr_event_with_placebo <- function(model, placebo_model, prefix = "rel_day_f::") {
  ...
  # Placebo model (no CI, just the line)
  placebo_df <- tidy(placebo_model, conf.int = FALSE) %>%
    filter(startsWith(term, prefix)) %>%
    mutate(
      rel_day = as.integer(sub(prefix, "", term)),
      RR  = exp(estimate)
  ...
  ggplot(rr_df, aes(x = rel_day, y = RR)) +
    geom_hline(yintercept = 1, linetype = "dashed") +
    # Main model CI band + line + points
    geom_ribbon(aes(ymin = LCI, ymax = UCI), fill = "grey80", alpha = 0.7) +
    geom_line(linewidth = 0.9, color = "black") +
    geom_point(size = 2, color = "black") +
    # Placebo: single red line, no ribbon/points
    geom_line(data = placebo_df, aes(x = rel_day, y = RR), color = "red", linewidth = 0.9) +
    # Make sure the x-axis includes all rel_day values from both datasets
    scale_x_continuous(breaks = sort(unique(c(rr_df$rel_day, placebo_df$rel_day)))) +
    labs(
      x = "Days relative to impact (lag/lead)",
      y = "Rate ratio (RR)",
      title = "RR by lag/lead day: main model (with 95% CI) + placebo (red line)"
    ) +
    theme_minimal()
}

# run it
plot_rr_event_with_placebo(m_event_55kt, m_event_55kt_placebo)
```
...
# --- Helper to build pre/post summaries for one model ---
...
# --- Define pre/post windows (adjust as needed) ---
pre_days  <- -8:-1
post_days <-  1:8

# --- Compute averages for both models ---
res_both <- bind_rows(
  summarize_prepost(m_event_64kt,          "Main (55kt)", pre_days, post_days),
  summarize_prepost(m_event_55kt_placebo2,  "Placebo",     pre_days, post_days)
)

# --- Plot: points + ±95% CI hash marks for BOTH models ---
dodge_width <- 0.45
pd <- position_dodge(width = dodge_width)
colors <- c("Main (55kt)" = "black", "Placebo" = "red")

p <- ggplot(res_both, aes(x = period, y = RR, color = model)) +
  geom_hline(yintercept = 1, linetype = "dashed") +
  geom_errorbar(aes(ymin = RR_L, ymax = RR_U),
                width = 0.12, position = pd, linewidth = 0.8) +
  geom_point(position = pd, size = 3) +
  scale_color_manual(values = colors, name = NULL) +
  labs(
    x = "Window",
    y = "Rate ratio (RR)",
    title = "Average pre/post RR with 95% CI: Main vs Placebo"
  ) +
  theme_minimal()
