...
## Write out TIC data frame
```{r}
saveRDS(TIC.sf, "data/outputs/Results/TIC_34kt.rds")
```
...
5,851,050 all deaths
  478,814 cardio deaths
  123,545 respir deaths
  241,485 injury deaths
  182,882 neuropsych deaths
...
Export (optional)
```{r}
readr::write_csv(panel, "data/outputs/Deaths/daily_panel_64kt_age50.csv")
# panel <- readr::read_csv("data/outputs/Deaths/daily_panel_34kt_age70.csv")
```
...
```{r}
saveRDS(m_event, file = "models/m_event_64kt_age50.rds")
```

## Read in model results
```{r}
m_event_64kt <- readRDS("models/m_event_64kt.rds")
m_event_64kt_age50 <- readRDS("models/m_event_64kt_age50.rds")
m_event_64kt_age60 <- readRDS("models/m_event_64kt_age60.rds")
m_event_64kt_age70 <- readRDS("models/m_event_64kt_age70.rds")
m_event_64kt_age80 <- readRDS("models/m_event_64kt_age80.rds")

m_event_34kt_placebo <- readRDS("models/m_event_34kt_placebo.rds")
m_event_55kt <- readRDS("models/m_event_55kt.rds")
m_event_55kt_placebo <- readRDS("models/m_event_55kt_placebo.rds")
m_event_64kt <- readRDS("models/m_event_64kt.rds")
m_event_64kt_placebo <- readRDS("models/m_event_64kt_placebo.rds")
```
...
```{r}
# --- Compute pre-impact average (lag -8:-1) for All vs Age ≥60 at ≥64 kt ---
res_pre_64kt_groups <- bind_rows(
  summarize_pre_only(m_event_64kt,      "All ages", pre_days),
  summarize_pre_only(m_event_64kt_age50,  "Age ≥ 50", pre_days)
) |>
  mutate(group = factor(group, levels = c("All ages", "Age ≥ 50")))
...
p_subgroup <- ggplot(res_pre_34kt_groups, aes(x = group, y = RR)) +
  ...
  labs(
    ...
    y = "Pre-impact average RR (95% CI)",
    title = "Pre-impact average rate ratio for ≥34 kt: All ages vs Age ≥ 70",
