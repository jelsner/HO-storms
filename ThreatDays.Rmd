...
deaths_with_zip<- readRDS("data/outputs/Results/deaths_with_zip_34kt.rds")
```
...
Export (optional)
```{r}
readr::write_csv(panel, "data/outputs/Deaths/daily_panel_34kt_age70.csv")
...
```{r}
saveRDS(m_event, file = "m_event_34kt_age70.rds")
```

## Read in model results
```{r}
m_event_34kt <- readRDS("m_event_34kt.rds")
m_event_34kt_age70 <- readRDS("m_event_34kt_age70.rds")
...
```{r}
# --- Compute pre-impact average (lag -8:-1) for All vs Age ≥70 at ≥34 kt ---
res_pre_34kt_groups <- bind_rows(
  summarize_pre_only(m_event_34kt,      "All ages", pre_days),
  summarize_pre_only(m_event_34kt_age70,  "Age ≥ 70", pre_days)
) |>
  mutate(group = factor(group, levels = c("All ages", "Age ≥ 70")))
