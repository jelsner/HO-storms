...
## Summary of the procedures used in Tc modeling and merging of birth records (initially written by chatGPT using the code below then edited)
...
We download and import North Atlantic tropical cyclone track data from the NOAA IBTrACS dataset. The data includes storm path geometries and attributes like wind speed and pressure. The tracks are filtered to include only significant storms (e.g., hurricanes with wind speeds ≥55 knots) that occurred between 1981 and 2022. Missing values for the radius to maximum winds (RMW) are imputed in stages based on pedigree (storm ID) and minimum central pressure.

Each storm's track is buffered using the RMW to create polygonal wind swaths. These swaths are then spatially intersected with the state of Florida to identify storms affecting the region. Overlapping swaths from the same storm are dissolved to form one geometry per storm, and each is assigned a Saffir-Simpson category based on peak wind speed. Swath geometries are transformed into a geographic coordinate system for compatibility with later spatial operations.
...
Trimesters.sf <- Trimesters.df |>
  ...
  sf::st_as_sf(coords = c("final_lon", "final_lat"),
               crs = 4326) |>
  dplyr::select(ID = ID, Birth_Date = Date, DateT1s, DateT1e,
...
births_storm_filtered <- chunk.sf |>
  ...
  dplyr::mutate(
    matched_storms = list(
      TIC.sf |>
        dplyr::filter(Date >= DateT2s & Date <= DateT2e))) |>
...
births_with_impacts <- births_with_impacts |>
  ...
  dplyr::select(ID, Birth_Date, DateT2s, DateT2e,  # change DateTXs, DateTXe
...
Combine with all births and remove the geometry column
```{r}
start_time <- Sys.time()
...
births_without_impacts <- births_without_impacts |>
  ...
  dplyr::select(ID, Birth_Date, DateT2s, DateT2e, Storm_Name, Storm_Category, Storm_Effect) # change DateTXs & DateTXs
...
combined_data_wide.dt <- dcast(
  ...
  formula = ID + Birth_Date + DateT2s + DateT2e ~ row_id, 
  value.var = c("Storm_Name", "Storm_Category", "Storm_Effect") #change DateTXs and DateTXe
)
Sys.time() - start_time
```

Combine with all data. Start by ordering births according to the ID in `All.dt` then column bind to `All.dt`
```{r}
...
All_2nd_Trimester_Storm_Effects.dt <- cbind(All.dt, combined_data_wide_ordered.dt)
...
```{r}
data.table::fwrite(All_2nd_Trimester_Storm_Effects.dt, 
                   file = "data/outputs/All_2nd_Trimester_Storm_Effects.csv")
