...
births_storm_filtered <- chunk.sf |>
  ...
  dplyr::mutate(
    matched_storms = list(
      TIC.sf |>
        dplyr::filter(Date >= DateT3s & Date <= DateT3e))) |>
...
births_with_impacts <- births_with_impacts |>
  ...
  dplyr::select(ID, Birth_Date, DateT3s, DateT3e,  # change DateTXs, DateTXe
...
births_without_impacts <- births_without_impacts |>
  ...
  dplyr::select(ID, Birth_Date, DateT3s, DateT3e, Storm_Name, Storm_Category, Storm_Effect) # change DateTXs & DateTXs
...
combined_data_wide.dt <- dcast(
  ...
  formula = ID + Birth_Date + DateT3s + DateT3e ~ row_id, 
...
All_3rd_Trimester_Storm_Effects.dt <- cbind(All.dt, combined_data_wide_ordered.dt)
...
data.table::fwrite(All_2nd_Trimester_Storm_Effects.dt, 
                   file = "data/outputs/All_3rd_Trimester_Storm_Effects.csv")
