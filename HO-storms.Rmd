...
Keep only births occurring inside the union of all threat/impact/cleanup (TIC) polygons. We assume the trimester dates have the same spatial location (the mother did not relocate during pregnancy). This is not very helpful for Florida using all storms
```{r}
withIn <- Births.sf |>
...
Filter births temporally by trimester start and end dates and spatially by storm TIC zones
```{r}
start_time <- Sys.time()
#keep_rows <- sample.int(nrow(Births.sf), size = 5000)

num_obs <- nrow(Births.sf)
#num_obs <- 500000  #for checking to see if things are working
chunk_size <- 50000
...
# 2. Perform spatial join to check if birth locations fall within the storm TIC zones
births_with_impacts <- sf::st_join(births_storm_filtered, TIC.sf, join = sf::st_within)

# 3. Keep only records where a birth was inside a storm TIC zone during the specified trimester
...
Merge with all births and remove the geometry
```{r}
...
# drop geometry and add dummy columns
