...
The situations are different for Erin, Irma, and Ian so it is not clear what to do. One way is simple to remove these storms from further consideration
```{r}
...
n_new <- 3    # Number of new rows to create for each feature (row) times 2
...
```{r}
hurricanes <- warning_advisories |>
  dplyr::filter(phenomena == "HU") 
```

Florida countiues and zone shapefiles
...
Note that the `hurricanes` dataset and the `zones.sf` dataset have different nomenclature for the zone names, where the `hurricanes` dataset uses the "ugc" column and the `zones.sf` dataset uses the "STATE_ZONE" column. Also, the zone names in the `hurricane` dataset includes the letter "z" in the designation so we need to create a new column in the hurricanes dataset that removes the "z" from the ugc column.
```{r}
...
```{r}
unique(hurricanes$ugc)

unique(zones.sf$STATE_ZONE)
```

Join `hurricanes` data with `zones.sf` shapefile
```{r}
hurricanes_zones <- zones.sf |>
  dplyr::left_join(hurricanes, by = c("STATE_ZONE" = "ugc"))

head(hurricanes_zones)
```

Create dates from character strings
```{r}
hurricanes_zones$utc_issue <- as.Date(hurricanes_zones$utc_issue)  
hurricanes_zones$utc_expire <- as.Date(hurricanes_zones$utc_expire)  
hurricanes_zones <- hurricanes_zones |>
  dplyr::arrange(desc(utc_issue))
```

```{r}
hurricanes_zones2 <- hurricanes_zones[, !(names(hurricanes_zones) %in% c("utc_polygon_begin", "utc_polygon_end", "utc_prodissue", "utc_init_expire", "utc_updated"))]
```
