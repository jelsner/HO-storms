...
Get corresponding birth and heat data by concatenating the `.Rdata` files in the folder `21_definition_final_re`
```{r}
start_time <- Sys.time()
...
births_with_impacts <- births_with_impacts |>
  ...
  dplyr::select(ID, Birth_Date, DateT1s, DateT1e,
...
births_without_impacts <- births_without_impacts |>
  ...
  dplyr::select(ID, Birth_Date, DateT1s, DateT1e, Storm_Name, Storm_Category, Storm_Effect)
...
merged_data.dt[, row_id := sequence(.N), by = ID]

# fast long to wide for data tables
merged_data_wide.dt <- dcast(
  data = merged_data.dt, 
  formula = ID + Birth_Date + DateT1s + DateT1e ~ row_id, 
  value.var = c("Storm_Name", "Storm_Category", "Storm_Effect")
)
```

Order births according to the ID in `All.dt` then column bind to `All.dt`
```{r}
merged_data_wide_ordered.dt <- merged_data_wide.dt[order(match(merged_data_wide.dt$ID, All.dt$ID))]

All_1st_Trimester_Storm_Effects.dt <- cbind(All.dt, merged_data_wide_ordered.dt)
```

Write out the data table to a csv file
```{r}
data.table::fwrite(All_1st_Trimester_Storm_Effects.dt, 
                   file = "data/outputs/All_1st_Trimester_Storm_Effects.csv")

# read it back
All_1st_Trimester_Storm_Effects.dt <- data.table::fread(file = "data/outputs/output.csv")
```
