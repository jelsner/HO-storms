...
The situations are different for Erin, Fay, and Irma so it is not clear what to do. One way is simple to remove these storms from further consideration. Here we keep them
```{r}
...
```{r}
Births.df <- data.table::fread(file = "data/all_births_hurricane.csv")
```

7.2 million births. Time period is 1981-1-1 until 2022-12-31. Only dates are given not times

Alternative birth data set from Jihoon posted on Teams March 31, 2025 in General > Data > Florida > 1_FL > Jihoon > 0_birth_Emily > 20_definition_final > birth_final_all_final_xxxx.Rdata where xxxx is year

Load in the `.Rdata` files
```{r}
load("data/20_definition_final/birth_all_final_1981.Rdata")
load("data/20_definition_final/birth_all_final_1982.Rdata")

load("data/21_definition_final_re/combine_data_all1981.Rdata")
```
...
Checks on file for match with Tulane folks
```{r}
X <- Births.df |>
  dplyr::filter(DATE_OF_BIRTH >= as.Date("2000-01-01") & DATE_OF_BIRTH <= as.Date("2000-12-31"))
```
...
Keep only births falling within the range of storm years
```{r}
withIn <- Births.sf$Year_of_Birth >= storm_year_range[1] & Births.sf$Year_of_Birth <= storm_year_range[2]
Births.sf <- Births.sf[withIn, ]
```

Down to 6.8 million births

# Start of code that parses by trimester

Keep only births in which either the trimester start or end months are within the storm season. Here we start with the third trimester
```{r}
withIn <- Births.sf$Month_of_T1e %in% storm_months | Births.sf$Month_of_T1s %in% storm_months
Births.sf <- Births.sf[withIn, ]
```

Down to 5.4 million births

Keep only births occurring inside the union of all effect (threat/impact/cleanup) polygons. We assume the trimester dates have the same spatial location (the mother did not relocate during pregnancy). This is not very helpful in reducing the number of births in the record using all storms since storms over these years cover almost the entire state of Florida
```{r}
withIn <- Births.sf |>
...
Check using a map. Sample 1,000 birth records
```{r}
...
Filter births temporally by trimester start and end dates and spatially by storm effect zones. This takes about 6.5 hours (3rd trimester) on my Apple M1 Pro with 32G memory. It took 4.3 hours on my Apple M4 (1st trimester)
```{r}
start_time <- Sys.time()
#keep_rows <- sample.int(nrow(Births.sf), size = 5000)

num_obs <- nrow(Births.sf)
#num_obs <- 500000  #check to see if things are working using a smaller subset of the data
chunk_size <- 50000
...
# 1 Filter births by trimester date range
births_storm_filtered <- chunk.sf |>
  dplyr::rowwise() |>
  dplyr::mutate(
    matched_storms = list(
      TIC.sf |>
        dplyr::filter(Date >= DateT1s & Date <= DateT1e))) |>
...
Merge with all births and remove the geometry column
```{r}
# which birth IDs are unique
unique_Birth_IDs <- unique(births_with_impacts$Birth_ID)

# is the birth ID not in the births_with_impacts data
X <- !Births.sf$Birth_ID %in% unique_Birth_IDs
births_without_impacts <- Births.sf[X,]
...
births_without_impacts <- births_without_impacts |>
  ...
  dplyr::select(Birth_ID, Birth_Date, DateT3s, Storm_Name, Storm_Category, Storm_Effect)

# merge and arrange by increasing birth dates
merged_data <- births_with_impacts |>
  sf::st_drop_geometry() |>
  rbind(births_without_impacts) |>
  dplyr::arrange(Birth_Date)
```

Cast to a wider format
```{r}
library(data.table)

# make the data frame a data table
merged_data.dt <- merged_data |>
  as.data.table()

# Create unique index per birth in the data table
merged_data.dt[, row_id := sequence(.N), by = Birth_ID]

# fast long to wide for data tables
merged_data_wide.dt <- dcast(
  data = merged_data.dt, 
  formula = Birth_ID + Birth_Date + DateT3s ~ row_id, 
  value.var = c("Storm_Name", "Storm_Category", "Storm_Effect")
)
```

Order births according to the original birth data ID
```{r}
#reorder based on Births.sf$Birth_ID
Births_with_storm_info_1st_trimester <- merged_data_wide.dt[order(match(merged_data_wide.dt$Birth_ID, Births.sf$Birth_ID))]

#reorder based on Births.df$Birth_ID
names(merged_data_wide.dt)[1] <- "ID"
Births_with_storm_info_1st_trimester2 <- merge(Births.df, merged_data_wide.dt, by = "ID", all.x = TRUE)  # Ensures all IDs are kept
setorder(Births_with_storm_info_1st_trimester2, ID)
```

Concatenate the .Rdata files in the folder `21_definition_final_re`, change the column labeled `ID` to `Birth_ID` then join the Births_with_storm_info_3rd_trimester by `Birth_ID`
```{r}
files <- list.files(path = "data/21_definition_final_re", pattern = "\\.Rdata$", full.names = TRUE)

#concatenated_df <- do.call(rbind, lapply(files, function(file) {
#  df <- get(load(file))  # Load the data frame from .Rdata
#  return(df)             # Ensure it's returned for binding
#}))  # error: vector memory limit of 32 Gb

#concatenated_dt <- rbindlist(lapply(files, function(file) {
#  df <- get(load(file))
#  as.data.table(df)
#}), use.names = TRUE, fill = TRUE) # error: vector memory limit of 32 Gb
...
for (i in seq(1, length(files), by = batch_size)) {
  ...
  batch_df <- do.call(rbind, lapply(batch_files, function(file) {
    df <- get(load(file))
    return(df)
  }))

  # Save intermediate results to disk to free memory
  save(batch_df, file = paste0("batch_", i, ".Rdata"))
  
  # Remove batch from memory to free space
  rm(batch_df)
  gc()  # Garbage collection
}

# Later, combine saved batch files
batch_files <- list.files(pattern = "batch_.*\\.Rdata$")
final_df <- do.call(rbind, lapply(batch_files, function(file) {
  get(load(file))
}))

load("data/21_definition_final_re/combine_data_all2022.Rdata")
#save(final_df, file = "final_combined.RData")
```

Write out the data table to a csv file
```{r}
data.table::fwrite(Births_with_storm_info_3rd_trimester, 
                   file = "data/outputs/Births_with_storm_info_3rd_trimester.csv")

# read it back
Births_with_storm_info_3rd_trimester <- data.table::fread(file = "data/outputs/output.csv")
```
