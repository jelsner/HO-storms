...
Filter births temporally by trimester start and end dates and spatially by storm effect zones. This takes about 6.5 hours on my Apple M1 Pro with 32G memory
```{r}
start_time <- Sys.time()
...
Births_with_storm_info_3rd_trimester <- merged_data_wide.dt[order(match(merged_data_wide.dt$Birth_ID, Births.sf$Birth_ID))]

#reorder based on Births.df$Birth_ID
names(merged_data_wide.dt)[1] <- "ID"
Births_with_storm_info_3rd_trimester2 <- merge(Births.df, merged_data_wide.dt, by = "ID", all.x = TRUE)  # Ensures all IDs are kept
setorder(Births_with_storm_info_3rd_trimester2, ID)
```
