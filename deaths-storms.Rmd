...
Align coordinate reference systems and clean geometries
```{r}
library(data.table)

# Align CRS: bring TIC polygons to zips CRS
tic_aligned  <- st_transform(TIC.sf,  st_crs(zctas_fl))
zcta_valid   <- st_make_valid(zctas_fl) |> 
...
Note death IDs are unique! Time period is 1981-1-1 until 2022-12-31. Only dates are given not times
...
Note death IDs are NOT unique!

Create a simple feature data frame with point geometry from the original data frame
```{r}
Deaths.sf <- Deaths.df |>
#  mutate(Date = lubridate::as_date(DATE_OF_DEATH)) |>
  st_as_sf(coords = c("final_lon", "final_lat"),
           crs = 4326) |>
  select(Death_ID = ID, Death_Date = Date)
```

Filter deaths temporally by date of death and spatially by storm effect zones. It took 2.42 hours on my Apple M4 for all deaths (1981-2022)
```{r}
start_time <- Sys.time()
