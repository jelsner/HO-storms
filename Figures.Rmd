...
if (!"IBTrACS.NA.list.v04r00.lines.shp" %in% list.files(here::here("data"))) {
...
track_daily_points <- do.call(
  rbind,
  lapply(seq_len(nrow(tracks_ex)), function(i) {
    xy <- st_coordinates(tracks_ex[i, ])[1, c("X", "Y")]
    data.frame(
      ISO_TIME = tracks_ex$ISO_TIME[i],
      date = as.Date(tracks_ex$ISO_TIME[i]),
      x = xy["X"],
      y = xy["Y"]
    )
  })
  ...
  slice(1) %>%
  ungroup() %>%
  mutate(date_lab = format(date, "%b %d"),
         label_dx = 45000,
         label_dy = case_when(
           date_lab == "Oct 09" ~  28000,
           date_lab == "Oct 10" ~  12000,
           TRUE                 ~  26000
         ))
...
pA <- ggplot() +
  ...
  geom_point(data = track_daily_points,
             aes(x = x, y = y),
             inherit.aes = FALSE,
             color = "#2C3E50", fill = "white",
             shape = 21, size = 1.6, stroke = 0.45) +
  geom_text(data = track_daily_points,
            aes(x = x + label_dx, y = y + label_dy, label = date_lab),
            inherit.aes = FALSE,
            hjust = 0, vjust = 0.5,
            size = 2.1, color = "#2C3E50") +
...
# ======================
# Panel B: Inset as a full panel to the right (ZCTA outlines that intersect swath)
# ======================

# Compute intersecting ZCTAs (in UTM)
...
# Tight bbox around the intersecting ZCTAs with ~20 km padding
...
# ======================
# Panel C: ZCTA × day calendar (Michael-only), full width below
# ======================

# Build ZCTA × date grid for ±8 days around Date0 (only intersecting ZCTAs)
...
# Reduce to a readable number of ZCTA rows (ordered north→south)
max_rows <- 60L
...
pC <- ggplot(cal_michael) +
  ...
  labs(
    title = "C. ZCTA × day exposure calendar",
    x = NULL, y = "ZCTA"
  ) +
  theme(
    plot.title      = element_text(face = "bold"),
