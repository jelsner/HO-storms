...
# Florida zips. National ZCTAs (2020 vintage is often best for modern work)
...
# Keep ZCTAs that intersect Florida
...
all_dates <- as.IDate(seq(as.Date(start_date), as.Date(end_date), by = "day"))
...
All deaths (1985-2022), >=55 kt storms, and +/- 1 day 1.7 hr
```{r}
start_time <- Sys.time()
...
Collapse `deaths_with_impacts` to one row per death, keeping the death occurring with the highest category storm irrespective of effect. For example, TS Bonnie and Cat 4 Charlie on 2004-08-13 was Cleanup on Bonnie and Impact on Charlie. Not needed for 55+ winds
```{r}
...
This tells us that in this sample the statewide daily death rate is somewhat higher on storm-effect days (all-cause deaths, 34+ kt winds). But a lot is going on: time and spatial trends, seasonality, autocorrelation. We need a model that accounts for these things
...
Daily death counts by zip. It doesn't make much sense to use daily average age or daily percent male
```{r}
...
Export (optional)
```{r}
readr::write_csv(zip_day, "data/outputs/Deaths/zip_day_34kt_1day.csv")

zip_day <- readr::read_csv("data/outputs/Deaths/zip_day_55kt_1day.csv")
```
...
Add strata variables. Note here we include only months during the hurricane season (May-November)
```{r}
dat <- zip_day %>%
