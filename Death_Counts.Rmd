...
Files received from Jihoon on November 27, 2025. We run these files sequentially, merging with storms and fitting models separately for each subset. Since attributes (age, sex, etc) are at the individual level and we are modeling daily counts we can't use these as simple covariates. Use 1985-01-01 for 55+ storms
```{r}
...
Filter deaths by date of death and storm effect zones. It took 6.5 hours on my Apple M4 using all deaths (1981-2022), >= 34 kt storms, and +/- 7 days on either side of impact day
```{r}
...
st_drop_geometry(deaths_all_effects) %>%
  readr::write_csv("data/outputs/Deaths/All_Deaths_Storm_Effects_34kt_7day.csv")

# To GeoPackage
st_write(
  deaths_all_effects,              # full sf object with long names
  dsn = "data/outputs/Deaths/All_Deaths_Storm_Effects_34kt_7day.gpkg",
  layer = "All_Deaths_Storm_Effects_34kt_7day",
  delete_dsn = TRUE
)
```
